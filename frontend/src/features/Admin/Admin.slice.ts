  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateConsumption.fulfilled, (state, { payload }) => {
      ...
      const categoryIndex = state.categories.findIndex(
        (value) => !!value.consumptions.find((cons) => cons.id === payload.id)
      );
      const consumptionIndex = state.categories[
        categoryIndex
      ].consumptions.findIndex((value) => value.id === payload.id);
    ...
    builder.addCase(deleteConsumption.fulfilled, (state, { payload }) => {
      ...
      const categoryIndex = state.categories.findIndex(
        (value) => !!value.consumptions.find((cons) => cons.id === payload)
      );
      const consumptionIndex = state.categories[
        categoryIndex
      ].consumptions.findIndex((value) => value.id === payload);
...
export const selectedCategoryId = (state: RootState): number | undefined =>
  state.admin.selectedCategoryId;
export const selectedConsumptionId = (state: RootState): number | undefined =>
  state.admin.selectedConsumptionId;
export const createConsumptionOpen = (state: RootState): boolean =>
  state.admin.createConsumptionOpen;
export const createCategoryOpen = (state: RootState): boolean =>
  state.admin.createCategoryOpen;
export const createConsumptionCategoryId = (
  state: RootState
): number | undefined => state.admin.createConsumptionCategoryId;
export const selectedCategory = (state: RootState): Category | undefined =>
  state.admin.categories.find((c) => c.id === state.admin.selectedCategoryId);
export const selectedConsumption = (
  state: RootState
