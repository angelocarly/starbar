...
export const updateConsumption = createAsyncThunk<UpdateConsumptionRequest, { name: string, price: number }, { state: RootState }>(
	"admin/updateConsumption",
	async ({ name, price }, { getState }) => {
		const oldConsumption = selectedConsumption(getState());
		if (!oldConsumption) {
			throw new Error("Consumption is undefined");
		}
		const result = await updateConsumptionAPI({ id: oldConsumption?.id, name, price });
...
}, { name: string, price: number }, { state: RootState }>(
	"admin/createConsumption",
	async ({ name, price }, { getState }) => {
