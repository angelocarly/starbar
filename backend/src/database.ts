...
const DBSOURCE = "./src/db/excuze.db";
        ...
        db.run(`CREATE TABLE category (category_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE);`,
            (err) => {
                if (err) {
                    ...
                    const insert = 'INSERT INTO category (category_id, name) VALUES (?, ?)';
        ...
        CREATE TABLE consumption (
            ...
            err => {
                if (err) {
                    ...
                    // Table just created, creating some rows
                    const insert = 'INSERT INTO consumption (name, price, category_id) VALUES (?, ?, ?)';
